...
**In Trae IDE:** Skills are auto-discovered from `.trae/skills/`. Use the `Skill` tool with the skill name (e.g., `Skill: "state-machine"` — no namespace prefix). See `references/trae-tools.md` for tool mapping.
...
### Installing GodotPrompter for Trae IDE

Skills are installed to `.trae/skills/` in your workspace. The recommended approach is to copy skill folders from the GodotPrompter repository:

```powershell
# Windows (PowerShell) — from your workspace root:
New-Item -ItemType Directory -Force -Path ".trae\skills" | Out-Null
robocopy "path\to\GodotPrompter\skills" ".trae\skills" /E
```

Each skill folder must be a direct child of `.trae/skills/` (e.g., `.trae/skills/state-machine/SKILL.md`). The AI agent auto-discovers skills and uses the `description` frontmatter to determine when to invoke them.

> **Agent support note:** Trae's `Task` tool uses built-in subagent types (`search`, `general_purpose_task`) and does not support custom agent registration via markdown files. The 9 GodotPrompter agent definitions (`agents/*.md`) serve as reference guidance — include relevant agent instructions in the task description when dispatching subagents.
...
> **Trae compatibility:** Trae does not support custom agent registration via markdown. When using `Task` with `general_purpose_task`, include the relevant agent's guidance in the task description.
...
- [`references/trae-tools.md`](references/trae-tools.md) — Trae IDE
